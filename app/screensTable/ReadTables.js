...
const ReadTablesScreen = ({route, navigation}) => {
    ...
    async function getData(){
        if(LOCAL && LOCAL.code){
            ...
            .catch(error => { Constants.NOTIFY('ERROR', error.code, 'ReadTables/getData', error.message) })
        }
    }
    ...
    useLayoutEffect(() => {
        ...
        navigation.setOptions({
          title: trans('tablesLocal'),
          headerRight: ()=>(
            !privileges.create ? null :
            <HeaderRight params={right} onPressRight={()=>launchScreen('CreateTable', {callbackItem: callbackItem})}/> )
        });
    }, [navigation]);
    ...
    const callbackList = (value, index) => {
        launchScreen('UpdateTable', { table: value, index: index, callbackItem: callbackItem })
    }
...
const PlatformList = ({ props }) => {
    ...
    const paramsDataList = {
        ...
        paramsItem: {
            type: 'GRID',
            icon: { name: 'chair', library: 'FontAwesome5', color: colors.default, size: size.iconLarge },
...
export default ReadTablesScreen