...
const UpdateRecipeScreen = ({route, navigation}) => {
    ...
    const { recipe, index, callbackItem } = route.params
    const [ isLoading, setLoading ] = useState(false)
    const [ copy, setCopy ] = useState({...recipe})
    ...
    useEffect(() => {
        updateHeader()
        if( !update ){ 
            new Recipe().setValuesFromObject(copy, recipe)
            setLoading(false)
        }
    }, [update, navigation ])
    ...
    const onPressUpdate = async () => {
        ...
        if( ! exceptionsValidate && LOCAL.companyCode ){
            try {
                ...
                await FBController.FS_Update('RECIPES', copy.code, copy)
                navigation.goBack(null)
            } catch (error) {
                Constants.NOTIFY('ERROR', error.code, 'UpdateRecipe/onPressUpdate', error.message)
            }
        }else{
    ...
    const onPressDelete = () => {
        copy['enable'] = false
        FBController.FS_Update('recipeS', copy.code, copy)
