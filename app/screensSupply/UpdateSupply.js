...
const UpdateSupplyScreen = ({route, navigation}) => {
    ...
    const updateHeader = () => {
        const title = update ? 'supplyEdit' : 'supplyDetails'
    ...
    useEffect(() => {
        updateHeader()
        if( !update ){ 
            new Supply().setValuesFromObject(copy, supply)
            setLoading(false)
        }
    }, [update, navigation ])
    ...
    const onPressUpdate = async () => {
        ...
        if( ! exceptionsValidate ){
            try {
                setLoading(true)
                await FBController.FS_Update('SUPPLIES', copy.code, copy)
                navigation.goBack(null)
            } catch (error) {
                Constants.NOTIFY('ERROR', error.code, 'UpdateSupply/onPressUpdate', error.message)
            }
        }else{
    ...
    const onPressDelete = () => {
        copy['state'] = 'DELETED'
        copy['enable'] = false
        FBController.FS_Update('SUPPLIES', copy.code, copy)
        navigation.goBack(null)
    }
...
export default UpdateSupplyScreen
