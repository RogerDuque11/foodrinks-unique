...
const UpdateDepartmentScreen = ({route, navigation}) => {
    const { PROFILE, LOCAL } = Constants.SESION
    const { styles, colors, size, trans } = CurrentScheme()
    const { department, index, callbackItem } = route.params
    ...
    const updateHeader = () => {
        const title = update ? 'departmentEdit' : 'departmentDetails'
    ...
    useEffect(() => {
        updateHeader()
        if( !update ){ 
            new Department().setValuesFromObject(copy, department)
            setLoading(false)
        }
    }, [update, navigation ])
    ...
    const onPressUpdate = async () => {
        ...
        if( ! exceptionsValidate ){
            try {
                setLoading(true)
                setUpdate(false)
                await FBController.FS_Update('DEPARTMENTS', copy.code, copy)
                new Department().setValuesFromObject(department, copy)
                callbackItem('UPDATE', copy, index)
                setLoading(false)
                navigation.goBack(null)
            } catch (error) {
                Constants.NOTIFY('ERROR', error.code, 'UpdateDepartment/onPressUpdate', error.message)
            }
        }else{
    ...
    const onPressDelete = () => {
        copy['state'] = 'DELETED'
        FBController.FS_Update('DEPARTMENTS', copy.code, copy)
