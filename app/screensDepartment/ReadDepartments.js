...
const ReadDepartmentsScreen = ({route, navigation}) => {
    ...
    const privileges = {
        create: PROFILE.usertype === 'ROOT' || PROFILE.usertype === 'PARTNER' ? true : false,
    ...
    async function getData(){
        ...
        .then((departments)=>{ setData(departments) })
        .finally(()=>{ setLoading(false) })
        .catch(error => { Constants.NOTIFY('ERROR', error.code, 'ReadDepartments/getData', error.message) })
    }
    ...
    useLayoutEffect(() => {
        ...
        navigation.setOptions({
          ...
          headerRight: ()=>(
            ...
            <HeaderRight params={right} onPressRight={()=>launchScreen('CreateDepartment', {callbackItem: callbackItem})}/> )
        });
    }, [navigation]);
    ...
    const callbackList = (value, index) => {
        if(privileges.update){
            launchScreen('UpdateDepartment', { department: value, index: index, callbackItem: callbackItem })
        }
    }
...
export default ReadDepartmentsScreen