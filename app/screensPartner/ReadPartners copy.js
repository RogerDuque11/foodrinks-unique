...
const ReadPartnersScreen = ({route, navigation}) => {
    ...
    async function getData(){
        setLoading(true)
        await FBController.FS_Read('PARTNERS')
        .then((partners)=>{ setData(partners) })
        .finally(()=>{ setLoading(false) })
        .catch(error => { Constants.NOTIFY('ERROR', error.code, 'ReadPartners/getData', error.message) })
    }
    ...
    useLayoutEffect(() => {
        ...
        navigation.setOptions({
          ...
          headerRight: ()=>( <HeaderRight params={right} onPressRight={()=>launchScreen('CreateUser', {callbackItem: callbackItem, usertype: 'partner'})}/> )
        });
    }, [navigation]);
...
export default ReadPartnersScreen