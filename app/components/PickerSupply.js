...
const PickerSupply = (props) => {
    ...
    useEffect(() => {
        if (data.length === 0 && !disabled){ loadSupplies() }
    }, [ currentValue ])

    async function loadSupplies() {
        await FBController.FS_ReadBy('SUPPLIES', 'enable', '==', true,)
        .then((result)=>{ setData(result) })
        .finally(()=>{  })
        .catch(error => { Constants.NOTIFY('ERROR', error.code, 'PickerSupply/loadSupplies', error.message) })
    }
