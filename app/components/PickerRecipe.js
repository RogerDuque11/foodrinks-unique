...
const PickerRecipe = (props) => {
    ...
    useEffect(() => {
        if (data.length === 0 && !disabled){ loadRecipes() }
    }, [ currentValue ])

    async function loadRecipes() {
        await FBController.FS_ReadBy('RECIPES', 'enable', '==', true,)
        .then((result)=>{ setData(result) })
        .finally(()=>{  })
        .catch(error => { Constants.NOTIFY('ERROR', error.code, 'PickerRecipe/loadRecipes', error.message) })
    }
