...
const PickerPartner = (props) => {
    ...
    useEffect(() => {
        if (data.length === 0 && !disabled){ loadPartners() }
    }, [ currentValue ])

    async function loadPartners() {
        await FBController.FS_ReadBy('PARTNERS', 'enable', '==', true, 'ORIGIN')
        .then((result)=>{ setData(result) })
        .finally(()=>{  })
        .catch(error => { Constants.NOTIFY('ERROR', error.code, 'PickerPartner/loadPartners', error.message) })
    }
